...
{
    public class DashboardService : DomainService
    {
        public DashboardService(lmsDbContext dbContext) : base(dbContext) { }
        #region dashboard V2
        /// <summary>
        ...
        public async Task<IList<StatisticalModel>> Overview(StatisticalSearch baseSearch, tbl_UserInformation currentUser)
        {
            ...
            {
                //số lượng chứng chỉ đã đạt
                var listCertificate = await dbContext.tbl_Certificate.Where(x => x.Enable == true && x.UserId == currentUser.UserInformationId).ToListAsync();
                totalData = listCertificate.Count;
                data = new StatisticalModel
                {
                    Type = "Tổng chứng chỉ",
                ...
                //tỷ lệ vượt qua bài kiểm tra và bài thi
                var listExamResult = await dbContext.tbl_ExamResult.Where(x => x.Enable == true && x.StudentId == currentUser.UserInformationId).ToListAsync();
                double totalExam = listExamResult.Count(x => x.ExamPeriodId == null);
                double totalPassExam = listExamResult.Count(x => x.ExamPeriodId == null && x.IsPass == true);
                double ratePassExam = 0;
                if (totalExam > 0)
                ...
                //số lượng khóa học đang tham gia
                var listVideoCourseStudent = await dbContext.tbl_VideoCourseStudent.Where(x => x.Enable == true && x.UserId == currentUser.UserInformationId).ToListAsync() ?? new List<tbl_VideoCourseStudent>();
        ...
        public async Task<AppDomainResult<RatePassExamModel>> RatePassExam(StatisticalSearch baseSearch, tbl_UserInformation currentUser)
        {
            ...
            var listExamResult = await dbContext.tbl_ExamResult.Where(x => x.Enable == true && x.CreatedOn.Value.Month == time.Month && x.CreatedOn.Value.Year == time.Year).ToListAsync() ?? new List<tbl_ExamResult>();
            var totalRow = 0;
            ...
            if (currentUser.RoleId == (int)RoleEnum.admin)
            {
                ...
                foreach (var student in listStudent)
                {
                    ...
                    data.DepartmentName = "";
                    var department = listDepartment.FirstOrDefault(x => x.Id == student.DepartmentId);
                    if (department != null)
                        data.DepartmentName = department.Name;
        ...
        #region thời gian mỗi nhân viên xem video trong tháng
        ...
        #region thống kê kỳ thi
        public async Task<AppDomainResult<ExamPeriodStatistical>> StatisticalExamPeriod(StatisticalSearch baseSearch, tbl_UserInformation user)
        {
            if (baseSearch == null) baseSearch = new StatisticalSearch();
            var dashboardService = new DashboardService(dbContext);
            var time = GetTimeModel(baseSearch.Month, baseSearch.Year);
            var listExamPeriod = await dbContext.tbl_ExamPeriod.Where(x => x.Enable == true && x.CreatedOn.Value.Month == time.Month && x.CreatedOn.Value.Year == time.Year).OrderByDescending(x => x.Id).ToListAsync() ?? new List<tbl_ExamPeriod>();
            ...
            if (listExamPeriod.Count <= 0) return new AppDomainResult<ExamPeriodStatistical> { TotalRow = 0, Data = null };
            ...
            // Phân trang
            int startIndex = (baseSearch.PageIndex - 1) * baseSearch.PageSize;
            listExamResult = listExamResult.Skip(startIndex).Take(baseSearch.PageSize).ToList();
            ...
            if (user.RoleId == (int)RoleEnum.admin)
            {
                foreach (var item in listExamPeriod)
                {
                    ...
                    data.TotalJoined = listUserInExamPeriod.Count(x => x.ExamPeriodId == item.Id);
